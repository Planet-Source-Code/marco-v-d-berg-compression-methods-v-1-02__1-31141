Attribute VB_Name = "Cod_Fix128_Bottom"
Option Explicit
...
Public Sub Fix128B_Coder(ByteArray() As Byte)
    ...
    For X = 1 To Times7
        bytes(0) = 0
        For Y = 1 To 7
            bytes(0) = bytes(0) + ((2 ^ (7 - Y)) * (-1 * ((ByteArray(InpPos) And 1) > 0)))
            bytes(Y) = Int(ByteArray(InpPos) / 2)
            InpPos = InpPos + 1
        Next
        For Y = 0 To 7
...
Public Sub Fix128B_DeCoder(ByteArray() As Byte)
    Dim OutStream() As Byte
    Dim BitsVal As Byte
    Dim FileLenght As Long
    Dim Times8 As Long
    ...
    For X = 1 To Times8
        ...
        InpPos = InpPos + 1
        For Y = 1 To 7
            OutStream(OutPos) = (ByteArray(InpPos) * 2) + (1 * (-1 * ((BitsVal And (2 ^ (7 - Y))) > 0)))
            OutPos = OutPos + 1
            InpPos = InpPos + 1
        Next
    Next
